...
export async function sendLangchainMessage(
  ...
  try {
    ...
    const agentResult = await invokeGymzyAgent(userInput, mappedHistory, userId /*, onStreamChunk */);

    // Extract structured data from agent result
    let workoutData: { exercises: any[]; workoutId: string } | null = agentResult.workoutData || null;
    let toolCallsForUI: Array<{ name: string; parameters: any; result?: any }> | undefined = undefined;

    let contentToStream = agentResult.finalOutput;
